...
namespace RestauranteDB.Database
{
    public class DataSeeder
    {
        ...
        // Método para inserir dados iniciais evitando duplicações
        public void InserirDadosIniciais()
        {
            using (SqlConnection conn = db.GetDatabaseConnection())
            {
                ...
                SqlCommand clearCmd = new SqlCommand("DELETE FROM Venda; DELETE FROM Usos; DELETE FROM Ingredientes; DELETE FROM Fornecedor; DELETE FROM Prato; DELETE FROM Cliente;", conn);
                ...
                string[] descricaoPratos = {
                    ...
                    "Hambúrguer artesanal com creme gorgonzola, geleia de pimenta, onion rings e duas fatias de banco.",
                ...
                for (int i = 1; i <= 10; i++)
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO Prato (id, nome, descricao, valor, disponibilidade) VALUES (@id, @nome, @descricao, @valor, @disponibilidade)", conn);
                    cmd.Parameters.AddWithValue("@id", i);
                ...
                for (int i = 1; i <= 10; i++)
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO Fornecedor (id, nome, estado_origem) VALUES (@id, @nome, @estado)", conn);
                    cmd.Parameters.AddWithValue("@id", i);
                ...
                for (int i = 1; i <= 10; i++)
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO Ingredientes (id, nome, data_fabricacao, data_validade, quantidade, observacao) VALUES (@id, @nome, @data_fabricacao, @data_validade, @quantidade, @observacao)", conn);
                    cmd.Parameters.AddWithValue("@id", i);
                ...
                Console.WriteLine("10 elementos realistas cadastrados em cada tabela!");
            }
        }
    }
}
